...
func waitForIfaces(ifaceNames []string, timeout time.Duration, closure func() error) error {
	...
	// Run the given function to trigger changes
		...
		case update := <-notifyLink:
			if update.Header.Type == syscall.RTM_NEWLINK &&
				...
				if _, set := ifaceStates[ifaceName]; set {
					...
					// Return if all interaces are ready
					finished := true
	...
	return fmt.Errorf("timeout waiting for network interfaces")
}
